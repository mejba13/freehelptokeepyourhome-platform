        ...
        <flux:sidebar sticky stashable class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <flux:navlist variant="outline">
                ...
                <flux:navlist.group :heading="__('Content')" class="grid">
                    <flux:navlist.item icon="document-text" :href="route('admin.pages.index')" :current="request()->routeIs('admin.pages.*')" wire:navigate>
                        {{ __('Pages') }}
                    </flux:navlist.item>
                    <flux:navlist.item icon="chat-bubble-left-right" :href="route('admin.testimonials.index')" :current="request()->routeIs('admin.testimonials.*')" wire:navigate>
                        {{ __('Testimonials') }}
                    </flux:navlist.item>
                    <flux:navlist.item icon="photo" :href="route('admin.hero.index')" :current="request()->routeIs('admin.hero.*')" wire:navigate>
                        {{ __('Hero Sections') }}
                    </flux:navlist.item>
                    <flux:navlist.item icon="megaphone" :href="route('admin.banners.index')" :current="request()->routeIs('admin.banners.*')" wire:navigate>
                        {{ __('Banners') }}
                    </flux:navlist.item>
                    <flux:navlist.item icon="cursor-arrow-rays" :href="route('admin.ctas.index')" :current="request()->routeIs('admin.ctas.*')" wire:navigate>
                        {{ __('CTAs') }}
                    </flux:navlist.item>
                </flux:navlist.group>

                <flux:navlist.group :heading="__('Communications')" class="grid">
                    <flux:navlist.item icon="envelope" :href="route('admin.submissions.index')" :current="request()->routeIs('admin.submissions.*')" wire:navigate>
                        {{ __('Form Submissions') }}
                        @php $newCount = \App\Models\ContactSubmission::new()->count(); @endphp
                        @if($newCount > 0)
                            <flux:badge size="sm" color="red">{{ $newCount }}</flux:badge>
                        @endif
                    </flux:navlist.item>
                </flux:navlist.group>
